...
KoMultiText만 사용할 때의 GPU 기준 권장 시작값입니다.
...
KoMultiText + KOLD를 함께 쓰는 MVP 학습:

```python
!python -m hate_detector.train_binary \
  --datasets komultitext kold \
  --output-dir models/kc-electra-komultitext-kold-binary \
  --epochs 3 \
  --batch-size 8 \
  --max-length 128 \
  --harmful-class-weight 1.8
...
AI Hub 텍스트 윤리검증 데이터를 함께 쓰려면 AI Hub에서 데이터를 내려받아 Drive에 풀고 `--aihub-dir`에 JSON 폴더를 지정합니다.

```python
!python -m hate_detector.train_binary \
  --datasets komultitext kold aihub \
  --aihub-dir data/aihub_text_ethics \
  --output-dir models/kc-electra-combined-binary \
  --epochs 3 \
  --batch-size 8 \
  --max-length 128 \
  --harmful-class-weight 1.8
...
검증셋 recall 기준으로 best checkpoint를 자동 재로딩하고 싶을 때만 `--load-best-model-at-end`를 추가합니다. Colab에서 LayerNorm `gamma/beta` 경고가 나오면 이 옵션 없이 학습한 마지막 epoch 모델을 쓰는 편이 안전합니다.
