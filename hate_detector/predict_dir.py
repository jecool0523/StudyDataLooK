...
from .predict_csv import build_text, read_csv_with_fallback, read_threshold, score_texts
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-dir", default="analysis_data/hate_predictions")
...
def analyze_frame(
    ...
    model_scores, valid_mask = score_texts(model, tokenizer, texts, batch_size, max_length, min_text_chars)
    ...
    result["is_harmful"] = [int(harmful) for harmful in harmful_mask]
    result["matched_terms"] = [", ".join(item.matched_terms) for item in keyword_results]
    result["matched_categories"] = [", ".join(item.matched_categories) if item else "" for item in category_results]
    result["primary_category"] = [
        item.primary_category if item else "none"
        for item in category_results
    ]
    for label in CATEGORY_LABELS:
        result[f"type_{label}_score"] = [item.scores[label] if item else 0.0 for item in category_results]
        result[f"type_{label}"] = [
            int(item is not None and item.scores[label] >= 0.5)
            for item in category_results
        ]
...
def main() -> None:
    ...
    for csv_path in csv_files:
        ...
        result = analyze_frame(df, model, tokenizer, threshold, args.batch_size, args.max_length, args.min_text_chars)
        relative = csv_path.relative_to(input_dir)
        ...
        category_counts = {f"type_{label}": int(result[f"type_{label}"].sum()) for label in CATEGORY_LABELS}
        summaries.append(
            {
        ...
        print(f"{relative}: total={total} harmful={harmful} invalid_text={invalid}")
